...
// TODO figure out how we change maxdatatostore when two rules are using the
// same resource, because they'll both be using the same resourceholder.
func (r *ResourceManager) NewResourceHolder(
...
func (r *ResourceManager) gatherResource(parsedEvent *ParsedEvent,
	pid int) error {
	resourceHolder := r.NewResourceHolder(parsedEvent)
...
func (r *ResourceHolder) getData(parsedEvent *ParsedEvent) (uint64, error) {
	...
	if duration.Seconds() > 1 {
		...
		timeBackThen := timeNow - duration.Nanoseconds()
		index := int64(-1)
		entries := []*DataTimestamp{}
		// TODO what about if we don't have more than one entry?
		// should we throw an error?
		// what about bounds checking?
		for {
			...
			if err != nil || timeBackThen > dataTimestamp.nanoTimestamp {
				break
			}
		}
		if len(entries) <= 1 {
		...
		timeDataCovers := first.nanoTimestamp-last.nanoTimestamp+(interval.Nanoseconds())
