...
func (s SigarGetter) getMem(pid int, kind string) (uint64, error) {
	...
	return 0, fmt.Errorf("Unknown resource 'mem_%v'.", kind)
}
...
func (s SigarGetter) getSysMem(kind string) (uint64, error) {
	...
	if err := mem.Get(); err != nil {
		return 0, fmt.Errorf("Couldn't get %v sys mem: 'sys_mem_%v'.", kind,
			err.Error())
	}
	switch kind {
	case "total":
	...
	return 0, fmt.Errorf("Unknown resource 'sys_mem_%v'.", kind)
}
...
func (r *ResourceManager) gatherResource(pid int, resourceName string) error {
	for index, resourceHolder := range r.resourceHolders {
		if resourceHolder.pid == pid &&
			...
			if err := resourceHolder.gather(r.sigarInterface); err != nil {
				return err
			}
	...
	resourceHolder.gather(r.sigarInterface)
...
func (r *ResourceHolder) gather(sigarInterface SigarInterface) error {
