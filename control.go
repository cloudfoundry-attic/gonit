...
const (
	MONITOR_MODE_ACTIVE  = 0x0
	MONITOR_MODE_PASSIVE = 0x1
	MONITOR_MODE_MANUAL  = 0x2
)
...
func (c *Control) MonitorModeSetActive(process *Process) {
	...
	state.MonitorModeLock.Lock()
	defer state.MonitorModeLock.Unlock()
	if state.MonitorMode != MONITOR_MODE_ACTIVE {
		log.Printf("%q set monitor mode to active", process.Name)
		state.MonitorMode = MONITOR_MODE_ACTIVE
	}
}
...
func (c *Control) MonitorModeSetPassive(process *Process) {
	...
	state.MonitorModeLock.Lock()
	defer state.MonitorModeLock.Unlock()
	if state.MonitorMode != MONITOR_MODE_PASSIVE {
		log.Printf("%q set monitor mode to passive", process.Name)
		state.MonitorMode = MONITOR_MODE_PASSIVE
	}
}
...
func (c *Control) MonitorModeSetManual(process *Process) {
	...
	state.MonitorModeLock.Lock()
	defer state.MonitorModeLock.Unlock()
	if state.MonitorMode != MONITOR_MODE_MANUAL {
		log.Printf("%q set monitor mode to manual", process.Name)
		state.MonitorMode = MONITOR_MODE_MANUAL
	}
}
...
func (c *Control) IsMonitoringModeActive(process *Process) bool {
	...
	state.MonitorModeLock.Lock()
	defer state.MonitorModeLock.Unlock()
	return state.MonitorMode == MONITOR_MODE_ACTIVE
}
...
func (c *Control) IsMonitoringModePassive(process *Process) bool {
	...
	state.MonitorModeLock.Lock()
	defer state.MonitorModeLock.Unlock()
	return state.MonitorMode == MONITOR_MODE_PASSIVE
}
...
func (c *Control) IsMonitoringModeManual(process *Process) bool {
	...
	state.MonitorModeLock.Lock()
	defer state.MonitorModeLock.Unlock()
	return state.MonitorMode == MONITOR_MODE_MANUAL
}
