...
func NewTestProcess(name string, flags []string) *Process {
	daemon := helper.NewTestDaemon(name, flags, false)
	process := &Process{}
...
func TestActions(t *testing.T) {
	...
	name := "simple"
	process := NewTestProcess(name, nil)
	defer helper.Cleanup(&process.Daemon)
...
func TestDepends(t *testing.T) {
	...
	for i := 0; i < nprocesses; i++ {
		...
		dprocess := NewTestProcess(dname, nil)
		defer helper.Cleanup(&dprocess.Daemon)
