...
func TestGatherMem(t *testing.T) {
	...
	resourceVal, err := r.GetResource(1234, "mem_used", duration)
	if err != nil {
		t.Fatal(err)
	}
...
func TestIsValidResourceName(t *testing.T) {
	Setup()
	assert.Equal(t, true, r.IsValidResourceName("mem_used"))
