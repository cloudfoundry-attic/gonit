...
func (e *EventMonitor) setup(configManager *ConfigManager, api *API) error {
		...
		for _, process := range group.Processes {
			for actionName, actions := range process.Actions {
				for _, eventName := range actions {
					...
					if err := e.loadEvent(event, group.Name, process, actionName); err != nil {
...
func (e *EventMonitor) triggerAction(event *ParsedEvent,
	...
	switch event.action {
	case "stop":
		return e.api.Control.DoAction(event.processName, ACTION_STOP)
	case "start":
		return e.api.Control.DoAction(event.processName, ACTION_START)
	case "restart":
		return e.api.Control.DoAction(event.processName, ACTION_RESTART)
	case "alert":
...
func (e *EventMonitor) Start(configManager *ConfigManager, api *API) error {
	if err := e.setup(configManager, api); err != nil {
		return err
	}
	go func() {
	...
	for _, event := range e.events {
		...
		if event.processName == processName &&
			...
			if ruleTriggered {
				...
				if err := e.triggerAction(event, resourceVal); err != nil {
					log.Print(err)
				}
			}
		}
...
func (e *EventMonitor) validateInterval(parsedEvent *ParsedEvent) error {
	...
	durationRatio := parsedEvent.duration.Seconds() / parsedEvent.interval.Seconds()
...
func (e *EventMonitor) StopMonitoringProcess(p *Process) {
	p.monitorEventsLock.Lock()
	defer p.monitorEventsLock.Unlock()
	log.Printf("\"%v\" event monitoring disabled.\n", p.Name)
	p.MonitorEvents = false
}

func (e *EventMonitor) StartMonitoringProcess(p *Process) {
	p.monitorEventsLock.Lock()
	defer p.monitorEventsLock.Unlock()
	log.Printf("\"%v\" event monitoring enabled.\n", p.Name)
	e.CleanDataForProcess(p)
	p.MonitorEvents = true
}

func (e *EventMonitor) IsMonitoring(p *Process) bool {
	p.monitorEventsLock.Lock()
	defer p.monitorEventsLock.Unlock()
	return p.MonitorEvents
}
