...
func assertFileParsed(t *testing.T, configManager *ConfigManager) {
	...
	assert.NotEqual(t, "", configManager.Settings.RpcServer)
	assert.Equal(t, 0, configManager.Settings.PollInterval)
	assert.NotEqual(t, nil, configManager.Settings.Process)
	assert.Equal(t, "lolnit", configManager.Settings.Process.Name)
}
